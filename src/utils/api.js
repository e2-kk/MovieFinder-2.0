...
export const getAllMovies = async (pageNum) => {
  try {
    ...
    return response.data.results; // Returns a Promise that resolves to all movies
  } catch (error) {
...
export const getMoviesWithinCategory = async (category, pageNum) => {
  try {
    ...
    return response.data.results; // Returns a Promise that resolves to movies within genres
  } catch (error) {
    console.error("Error fetching movies:", error);
    return []; // Return an empty array in case of error
  }
